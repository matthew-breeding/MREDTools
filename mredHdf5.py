...
def hdf5RunManager(mred, options, bv=None, singleEventMode = False, function=None, saveDose = False, **kwargs):
    """
    This method sets up batch runs to incrementally save hdf5 files at regular intervals of nIons for both 
	beamOn mode and single event mode. 

    The mred and options instances are REQUIRED to be passed when calling the function; optionally, the 
	@batch_vars class can be passed for jobs submitted to a compute node. If there is an instance of 
	batch_vars, the run index for each file name is procedurally generated by adding the options.index
	and the batch_vars.index. This is useful for parallelized runs that are not necessarily submitted
	at the same time. 

    The options namespace MUST include the following attributes: 
	options.nIons      -->  the TOTAL number of particles to be simulated for the entire run
	options.nSaves     -->  number of individual save files; i.e. for nIons = 1000 and nSaves = 10, 
				    hdf5RunManager splits the simulation into 10 beamOn() commands
				    each with 100 particles simulated with a new data output file 
				    created for each of the 10 branches --> 10 total output files, 
				    each with data for 100 particles* (*IMPORTANT -- see note in 
				    options.retainAll for essential info)
	options.retainAll  -->  default value is False; for histograms that are autogenerated in MRED, 
				    (mred.autogenerate_histograms = True in the main script) the 
				    histogram values are reset for each mred.beamOn() command. However,
				    if the histograms are not autogenerated but instead manually 
				    created and attached to the sensitive volumes/detectors, the 
				    histogram tables in HDF5 files are automatically updated across
				    all mred.beamOn() commands. This means that you would only end up
				    with one output data file instead of 10 incremental files that would
				    still need to be combined in post-processing. 
				    It is better practice in general to explicitly name your sensitive 
				    regions and explicitly define your histogram parameters. An explicit
				    catch is built in to automatically toggle this flag based on the 
				    mred.autogenerate_histograms attribute. 
	options.runName    -->  self explanatory
	options.saveDir    -->  final save directory for data output. Must be valid path in /home or /scratch
	options.index      -->  the run index. For single runs, this is just 0; for multiple simulations
				    with the same initial conditions, this can be incremented to 
				    facilitate post-processing combinations.

    The HDF5 file attributes that are handled automatically here are the file_path, file_name,and setting 
	write_outputs = True. Every other mred.hdf5 option is left to the user to select within the main 
	script  ** include mred.hdf5.include_histograms (which defaults to False) **. 
	File names are incremented for each mred.beamOn() command (or each mred.runSingleEventMode). 

    Any **kwargs passed to the function are also automatically added to HDF5 output file's attributes table.
	This is especially useful if there are certain values of interest that don't require loading 
	all of the histograms into memory.

    At the end of each incremental run, the tarFile() method handles the file output behavior. For runs saving
	to /tmp/, the .hdf5 file is compressed and moved to the options.saveDir directory, with the files on 
	/tmp getting removed. If already saving at options.saveDir, the old .hdf5 files are removed after compression
	and the old .tar.gz files are removed only if options.retainAll == False (default behavior -- see note above)
    """
